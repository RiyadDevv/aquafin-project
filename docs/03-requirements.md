...
Het systeem moet op basis van historische neerslagdata en een trendberekening (lineaire regressie) overstromingsrisico's voorspellen voor het huidig jaar en de komende 5 jaar, per seizoen.
...
Het systeem moet een beheerder toelaten nieuwe materialen toe te voegen met naam, categorie en flood tool markering. Het systeem controleert op duplicaten binnen de actieve materialen.
...
- de naam van de technieker ingevuld is
- minstens één materiaal geselecteerd is
- een geldige leverdatum opgegeven is (verplicht en in de toekomst)
...
Het systeem moet automatisch detecteren of het huidige seizoen een verhoogd overstromingsrisico heeft op basis van de risicoforecast.
...
Het systeem moet tijdens een periode met hoog of gemiddeld overstromingsrisico:
- de relevante flood tools bovenaan de Materialen pagina plaatsen
- een aparte flood tools sectie tonen onderaan het Dashboard
...
Het systeem moet aanbevolen flood tools tonen met een duidelijke visuele markering (oranje "Flood Tool" badge).
...
- het risiconiveau van het huidige seizoen (kleurgecodeerde kaart)
- een beknopt overzicht van de komende seizoenen (seizoenskaarten)
- de aanbevolen flood tools bij hoog of gemiddeld risico (aparte sectie onderaan)
...
Het systeem moet een aparte pagina bevatten met een forecast tabel per seizoen voor het huidig jaar en de komende 5 jaar.
...
Het systeem moet een overzichtelijke materiaallijst tonen met zoek- en filteropties, en flood tools bovenaan bij verhoogd risico.
...
Het systeem moet een beheerderspagina bevatten voor het toevoegen en deactiveren van materialen, met toast feedback bij elke actie.
...
De applicatie geeft bij elke gebruikersactie zichtbare feedback via toast meldingen (rechtsonder, automatisch verdwijnend na 3,5 seconden) of inline foutmeldingen bij validatiefouten.
...
De risicoforecast moet consistent dezelfde resultaten geven bij dezelfde inputdata. De berekening is gebaseerd op de seizoensdrempels uit de opdracht en wordt on-the-fly uitgevoerd.
...
De businesslogica (risicoanalyse, aanbevelingen, materiaalbeheer, bestellingen) wordt verwerkt door een Node.js + Express backend die een REST API aanbiedt. Nodemon wordt gebruikt voor de development server (`npm start`).
...
### TR-04 - Seed data
De database wordt bij initialisatie gevuld met de historische neerslaggegevens (2004–2025), de materiaalcatalogus en de standaardcategorieën via `node db/seed.js`.
